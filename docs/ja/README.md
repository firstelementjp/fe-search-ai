...
- **ハイブリッド検索**: ベクトル検索とBM25キーワードランキングを組み合わせます。
- **複数AIプロバイダー対応**: OpenAI(GPT)、Google(Gemini)、Anthropic(Claude)を利用できます。
- **Embedding対応**: OpenAIまたはGoogleのEmbeddingモデルを利用できます。
- **Qdrant連携**: ベクトルデータベースとしてQdrant CloudまたはセルフホストのQdrantを利用できます。
- **日本語対応**: TinySegmenterまたはYahoo! JAPAN 日本語形態素解析APIを利用できます。
- **チャットUIのカスタマイズ**: ショートコードで埋め込み、またはフローティング表示できます。
- **自動更新対応**: GitHub Releasesからの自動更新に対応します。
- **開発者向けフック**: プロバイダー、プロンプト、同期、表示、GitHub更新挙動を拡張できます。

## 現在のリリース

バージョン1.1.0では、BM25、Qdrant、RRF、Cohereのランキング診断に使える検索トレーススコア詳細、古いまたは不完全なキーワードインデックスを見つけるための同期インデックス健全性メトリクス、検索トレース内のソース数メタデータ、およびRerank品質とパフォーマンスを調整するhybrid candidate limit設定を追加しています。
