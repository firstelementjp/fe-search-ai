...
| Hook                                                | Filtered value and additional arguments                            | Purpose                                                                                                                                                   |
| --------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fe_search_ai_privacy_provider_registry`            | `array $registry`, `array $settings`, `array $pro_settings`        | Changes provider data-handling metadata used by the Privacy tab and frontend notice.                                                                      |
| `fe_search_ai_active_privacy_recipients`            | `array $recipients`, `array $settings`, `array $pro_settings`      | Changes the recipients reported as active for the current configuration.                                                                                  |
| `fe_search_ai_privacy_config`                       | `array $config`, `array $settings`, `array $pro_settings`          | Changes the versioned frontend privacy, consent, and logging configuration.                                                                               |
| `fe_search_ai_validate_chat_consent`                | `bool $valid`, `string $token`, `WP_REST_Request $request`         | Validates required consent before a chat request. Default: `true`; Pro performs token validation when enabled.                                            |
| `fe_search_ai_conversation_log_mode`                | `string $mode`, `string $session_id`, `string $token`              | Selects `none`, `diagnostic`, or `analytics` conversation logging. Default: `none`.                                                                       |
...
| `fe_search_ai_log_retention_days`                   | `int $days`                                                        | Changes system-log retention. The filtered default comes from the `Log Retention (days)` setting (default: `30` days).                                    |
| `fe_search_ai_conversation_log_retention_days`      | `int $days`                                                        | Changes conversation-log retention. The filtered default comes from the Pro `Conversation log retention` privacy setting (default: `7` days).             |
| `fe_search_ai_retrieval_trace_retention_days`       | `int $days`                                                        | Changes retrieval-trace retention used by the daily rotation. The filtered default comes from the retrieval trace retention setting (default: `30` days). |
